...
(function() {
    ...
    $(document).ready(function() {
        ...
        (async function() {
            try {
                ...
                // Store results globally for history navigation
                window.Pa11yHistoryResults = results;
                // Render the latest result by default
                renderLocalResult(results[0]);
            } catch (err) {
    ...
    /**
     * Render a local result on the page. This function builds the
     * result detail section (tabs and panels) for a given result object
     * and inserts it into the DOM. It removes any prior rendered
     * section and hides the server-only aside. It also removes any
     * "no results" alerts to avoid duplicate links. This function is
     * exposed globally via window.renderLocalResult so that history.js
     * can call it when the user selects a run from the history list.
     *
    ...
    function renderLocalResult(result) {
        ...
        // Remove existing rendered section if present
        $('#local-result').remove();
        ...
        navHtml += '</ul>';
        // Compose section
        var sectionHtml = '<section class="col-md-9" id="local-result">';
        sectionHtml += '<h2 id="tabSectionHeading" class="crunch-top">Results</h2>';
        sectionHtml += navHtml + contentHtml;
        sectionHtml += '</section>';
        // Remove any 'no results' alerts containing run-task links
    ...
    // Expose the render function globally so history.js can call it
    window.renderLocalResult = renderLocalResult;
})();